...
pub fn main() !void {
    ...
    const musl_sysroot_path = args[1];
    const musl_version = try std.SemanticVersion.parse(args[2]);

    var sysroots_dir = try std.fs.cwd().openDir(musl_sysroot_path, .{});
    defer sysroots_dir.close();

    for (arches) |arch| {
        const abilist_dir_path = try std.fmt.allocPrint(arena, "{d}.{d}.{d}/{s}", .{
            musl_version.major,
            musl_version.minor,
            musl_version.patch,
            arch,
        ...
        const elf_bytes = try sysroots_dir.readFileAllocOptions(
            arena,
            try std.fmt.allocPrint(arena, "{s}/usr/lib/libc.so", .{arch}),
            100 * 1024 * 1024,
            3 * 1024 * 1024,
