...
const zig_targets = [_]ZigTarget{
    .{ .name = "evbarmv7", .arch = .arm, .abi = .eabi },
    .{ .name = "evbarmv7hf", .arch = .arm, .abi = .eabihf },
    .{ .name = "evbarmv7", .arch = .armeb, .abi = .eabi },
    .{ .name = "evbarmv7hf", .arch = .armeb, .abi = .eabihf },
...
pub fn main() !void {
        ...
        for (zig_targets) |target| {
            for (lib_names, 0..) |lib_name, lib_i| {
                ...
                while (lines_it.next()) |line| {
                    ...
                    gop.value_ptr.type[lib_i][target.getIndex()][verIndex(fs_ver)] = ty;
                }
            }
        }
    }
    ...
    for (zig_targets) |zt| {
        try w.print("{s}-netbsd-{s}\x00", .{ @tagName(zt.arch), @tagName(zt.abi) });
    }

    {
        ...
        while (i < fn_inclusions.items.len) {
            ...
            while (true) {
                ...
                var lib = inc.lib;
                lib |= 1 << 5; // NetBSD libraries are always unversioned.
                if (inc.weak) {
                    lib |= 1 << 6;
                }
                if (set_terminal_bit) {
